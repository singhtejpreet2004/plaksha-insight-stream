    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-accent/20 to-background">
      ...
      <header className="border-b bg-card/80 backdrop-blur-sm sticky top-0 z-50 shadow-sm">
        <div className="container mx-auto px-6 py-4">
          ...
          <h1 className="text-3xl font-bold text-foreground">Entry/Exit Monitoring</h1>
          <p className="text-muted-foreground mt-2">
            Track entry and exit patterns with intelligent flow analysis
          </p>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="border-2 border-dashed border-muted rounded-lg p-8 bg-muted/20">
              ...
              <h3 className="font-semibold text-lg mb-2">Gate {i}</h3>
              <p className="text-sm text-muted-foreground">Entry/Exit flow statistics will appear here</p>
            </div>
          ))}
        </div>
      </main>
